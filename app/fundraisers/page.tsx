      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {activeFundraisers.map((fundraiser) => (
            <Card key={fundraiser.id} className="flex flex-col overflow-hidden hover:shadow-lg transition-shadow h-full">
              {fundraiser.imageUrl && (
              ...
              <CardHeader>
                <CardTitle className="text-2xl font-bold leading-tight">
                  <Link href={`/fundraisers/${fundraiser.slug}`} className="hover:text-primary transition-colors">
                    {fundraiser.title}
                  </Link>
                </CardTitle>
                    ...
                    {fundraiser.events.length > 0 && (
                        <div className="flex items-center gap-2">
                            ...
                            <span>
                                ...
                                {fundraiser.events.length > 1 && ` + ${fundraiser.events.length - 1} more`}
                            </span>
                        </div>
                    )}
          ...
          {activeFundraisers.length === 0 && (
            <div className="col-span-full text-center py-12 text-muted-foreground">
              <p>No active fundraisers at the moment. Please check back later!</p>
            </div>
          )}
        </div>
      </div>
